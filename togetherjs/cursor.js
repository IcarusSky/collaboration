...
define(["jquery", "ui", "util", "session", "elementFinder", "tinycolor", "eventMaker", "peers", "templating", "chat"], function ($, ui, util, session, elementFinder, tinycolor, eventMaker, peers, templating, chat) {
  ...
  function documentClick(event) {
    ...
    //新增代码开始
    if (event.currentTarget.activeElement.id == "") {
      var element = event.target;
      var offset = $(element).offset();
      var offsetX = Math.floor(event.pageX - offset.left);
      var offsetY = Math.floor(event.pageY - offset.top);
      chat.submitHistoryMessage("点击X:" + offsetX + "点击Y:" + offsetY);
    } else {
      var id = event.currentTarget.activeElement.id
      if (id == "togetherjs-profile-button") {
        chat.submitHistoryMessage("点击用户设定");
      } else if (id == "togetherjs-share-button") {
        chat.submitHistoryMessage("点击邀请分享");
      } else if (id == "togetherjs-history-button") {
        chat.submitHistoryMessage("点击历史信息");
      } else if (id == "togetherjs-chat-button") {
        chat.submitHistoryMessage("点击聊天");
      } else if (id.indexOf("togetherjs-dock-element") != -1) {
        var name = event.currentTarget.activeElement.childNodes[1].attributes[0].ownerElement.innerText;
        chat.submitHistoryMessage("点击伙伴信息:" + name);
      }
    }
    //新增代码结束
