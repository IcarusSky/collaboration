...
define(["require", "jquery", "util", "session", "templates", "templating", "linkify", "peers", "windowing", "tinycolor", "elementFinder", "visibilityApi"], function (require, $, util, session, templates, templating, linkify, peers, windowing, tinycolor, elementFinder, visibilityApi) {
  ...
  ui.chat = {
    text: function (attrs) {
      if (attrs.typeOf == "history") {
        assert(typeof attrs.text == "string");
        assert(attrs.peer);
        assert(attrs.messageId);
        var date = attrs.date || Date.now();
        var lastEl = ui.container.find("#togetherjs-history .togetherjs-history-message");
        if (lastEl.length) {
        ...
        var el = templating.sub("history-message", {
          peer: attrs.peer,
          content: attrs.text,
          date: date
        });
        linkify(el.find(".togetherjs-history-content"));
        el.attr("data-person", attrs.peer.id)
          .attr("data-date", date)
          .attr("data-message-id", attrs.messageId);
        ui.chat.addHistory(el, attrs.messageId, attrs.notify);
      } else {
        assert(typeof attrs.text == "string");
        assert(attrs.peer);
        assert(attrs.messageId);
    ...
    //新增代码开始
    addHistory: deferForContainer(function (el, id, notify) {
      if (id) {
        el.attr("id", "togetherjs-history-" + util.safeClassName(id));
      }
      var container = ui.container.find("#togetherjs-history-messages");
      assert(container.length);
      var popup = ui.container.find("#togetherjs-history-notifier");
      container.append(el);
      ui.chat.scroll();
      var doNotify = !!notify;
      var section = popup.find("#togetherjs-history-notifier-message");
      if (notify && visibilityApi.hidden()) {
        ui.container.find("#togetherjs-notification")[0].play();
      }
      if (id && section.data("message-id") == id) {
